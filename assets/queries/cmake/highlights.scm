...
;; Source: https://github.com/nvim-treesitter/nvim-treesitter/blob/cf12346a3414fa1b06af75c79faebe7f76df080a/queries/cmake/highlights.scm
;;
;; Modification: (1) Vim `\c` case-insensitive prefixes in #match? regexes
;; rewritten as Rust-regex `(?i)` inline flags (the native engine rejects
;; `\c`); cmake keywords are case-insensitive, so the flag is load-bearing.
;; (2) `#lua-match?` predicates rewritten as `#match?` with Lua classes
;; (`%u`/`%d`) expanded to regex (`[A-Z]`/`[0-9]`) so the engine enforces
;; them — cmake highlighting is almost entirely predicate-driven.
